...
namespace TRODS
{
    class InGame : AbstractScene
    {
        ...
        public override void Update(float elapsedTime)
        {
            ...
            for (int i = 0; i < _mobs[_currentMap].Count; i++)
            {
                if (personnage.Attacks.ContainsKey(personnage.Action) && personnage.Attacks[personnage.Action].Position.Intersects(_mobs[_currentMap][i].DrawingRectangle))
                {
                    _mobs[_currentMap][i].ReceiveAttack(personnage.Attacks[personnage.Action].Damage);
                    personnage.Attack(personnage.Action);
                }
